...
The job now is to find out if among these people we could detect a particular pattern of connections that may mean the presence of communities emerging from the relationships. Let's compute some basic net stats to see if we can suspect the existence of communities.

## Clustering

If the nodes are clustering, it is a first sign that communities may be present. Think in terms in triangles and triplets. If you have one node connected to other two nodes, you have a triplet; if those other two nodes are also connected you have a closed triplet or a triangle, if not you have an open triplet.

So, there are actually two ways to compute a clustering coefficient; the first one based on a simple division between the triangles by the triplets counts in the whole graph, this is the *global clustering coefficient*:
...
Here, I compute the local version by actor:

```{r, eval=TRUE, message=FALSE}
transitivity(actors,type = "local")
```

Then, the average of the above values:
...
Since the minimum value is 1, you have 11 cliques of one node, 14 cliques of two nodes, and two cliques of three nodes.

Here, we can know who are in the biggest cliques:
...
```{r, warning=FALSE}
actors_eb <- cluster_edge_betweenness(actors,directed = F)
```
...
```{r}
modularity(actors_lv)
```

Girvan-Newman has a higher modularity, but the difference might not be significant.
